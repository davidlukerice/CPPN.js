...
(function() {
	...
	CPPN.Network.prototype = {
		...
		addInputNodes: function(numInputs) {
			...
			while(numInputs--)
				nodes.push(new CPPN.Node(inputType, null, 0));
		},
		...
		addBiasNode: function() {
			var nodes = this.nodes,
				...
				nodes.push(new CPPN.Node(biasType, null, 0));
		},
		...
		addOutputNodes: function(numOutputs) {
			...
			while(numOutputs--)
				nodes.push(new CPPN.Node(outputType, null, 1));
		},
		...
		/**
		 * Random chance of a type of mutation
		 **/
		randomMutation: function() {
			...
			if (index === 0)
				...
				this.mutateAllConnectionWeights(0.2);
		},
		...
		splitConnection: function(connection) {
			var hiddenType = nodeTypes.Hidden,
				...
				newDepth = connection.inNode.depth + connection.outNode.depth;
			newDepth/=2;
			...
			newNode = new CPPN.Node(hiddenType, null, newDepth);
